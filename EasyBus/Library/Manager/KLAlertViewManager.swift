...
class KLAlertViewManager {
    ...
    func showTopAlert(target targetView: UIView, size: CGSize = CGSize(width: ScreenSizeManager.screenWidth, height: 54), contentInset: UIEdgeInsets = .zero, text: String, icon: UIImage? = nil, completion:(() -> Void)? = nil){
        
        let topInset = ScreenSizeManager.safeAreaTopInset
        let initFrame = CGRect(x: 0, y: targetView.frame.origin.y - size.height, width: size.width, height: size.height + topInset)
        let finalFrame = CGRect(x: 0, y: targetView.frame.origin.y, width: ScreenSizeManager.screenWidth, height: initFrame.size.height)
    ...
    func showBottomAlert(target targetView: UIView, size: CGSize = CGSize(width: ScreenSizeManager.screenWidth, height: 45), text: String, icon: UIImage? = nil, completion:(() -> Void)? = nil){
        ...
        UIView.animate(withDuration: slideDuration, delay: 0, options: UIViewAnimationOptions(), animations: {
            ...
            smartAlertButton?.alpha = 1
            smartAlertButton?.frame = finalFrame
        }, completion: { [weak smartAlertButton] (completed) in
            if completed == true {
                UIView.animate(withDuration: self.slideDuration, delay: self.showDuration, options: UIViewAnimationOptions(), animations: {
                    [weak smartAlertButton] in
                    smartAlertButton?.alpha = 0
